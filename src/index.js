...
const params = {
  ...
  atmMultiplier: { value: 3000 },
  atmNightFade: { value: 0.1 },
  blendFactor: { value: 0.3 }
};

const earthRadius = 100;
const atomsRadius = 103;
const cloudRadius = 100.3;
...
let camera = createCamera(45, 10, 2000, { x: 0, y: 0, z: 300 });
...
let app = {
  async initScene() {
    ...
    this.dirLight.position.set(-500, 0, 300);
    ...
    let earthGeo = new THREE.SphereGeometry(earthRadius, 128, 128);
    ...
    let cloudGeo = new THREE.SphereGeometry(cloudRadius, 64, 64);
    ...
    let atmosGeo = new THREE.SphereGeometry(atomsRadius, 64, 64);
    let atmosMat = new THREE.ShaderMaterial({
      vertexShader: vertexShader,
      fragmentShader: fragmentShader,
      uniforms: {
        planetRadius: { value: earthRadius },
        atmosphereRadius: { value: atomsRadius },
    ...
    earthMat.onBeforeCompile = function (shader) {
      ...
      shader.fragmentShader = shader.fragmentShader.replace(
        ...
        diffuseColor.rgb *= max(1.0 - cloudsMapValue, 0.4 );
    ...
    gui.add(params.atmPowFactor, "value", 2.0, 7.0, 0.05).name("atmPowFactor");
    gui.add(params.atmMultiplier, "value", 0.0, 10000.0, 100).name("atmMultiplier");
    gui.add(params.atmNightFade, "value", 0.0, 1.0, 0.05).name("atmNightFade");
    gui.add(params.blendFactor, "value", 0.0, 1.0, 0.1).name("blendFactor");
